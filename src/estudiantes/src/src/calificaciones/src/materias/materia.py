class Materia:
    def __init__(self, codigo_materia, nombre_materia, id_profesor_asignado=None, creditos=None):
        self.codigo_materia = codigo_materia  # Clave primaria
        self.nombre_materia = nombre_materia
        self.id_profesor_asignado = id_profesor_asignado  # Clave foránea referenciando a PROFESOR
        self.creditos = creditos

    def mostrar_info(self):
        info = f"Código Materia: {self.codigo_materia}, Nombre: {self.nombre_materia}"
        if self.id_profesor_asignado:
            info += f", Profesor Asignado ID: {self.id_profesor_asignado}"
        if self.creditos:
            info += f", Créditos: {self.creditos}"
        print(info)
